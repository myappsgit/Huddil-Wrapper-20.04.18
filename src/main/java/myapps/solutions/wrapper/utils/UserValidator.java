...
public class UserValidator {
	...
	private static boolean validateUser(UserDetails user, ConstraintValidatorContext context) {
		...
		/*
		 * data = user.getMobileNo().trim(); if (TextUtils.isEmpty(data)) { isValid =
		 * false;
		 * context.buildConstraintViolationWithTemplate("NotEmpty.userDetails.mobileNo")
		 * .addPropertyNode("mobileNo") .addConstraintViolation(); } else if
		 * (!user.getMobileNo().matches(RegExs.indianMobileNo)) { isValid = false;
		 * context.buildConstraintViolationWithTemplate("Pattern.userDetails.mobileNo").
		 * addPropertyNode("mobileNo") .addConstraintViolation(); }
		 */
		...
		if (user.getUserSubscriptions().get(0).getUserType().getType().equalsIgnoreCase("service provider")) {
			isValid = validateField(user.getAddress(), context, "address")
					...
					&& validateField(user.getWebsite(), context, "website")
					&& validateField(user.getCompanyName(), context, "companyName") && isValid;
